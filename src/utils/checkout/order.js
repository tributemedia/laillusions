...
export const getCreateOrderData = ( order, products ) => {
	...
	return {
		...
		coupon_lines: order?.couponLines,
		status: order?.status,
	};
};
 ...
 * @returns {Promise<{orderId: null, error: string}>}
 */
export const updateTheOrder = async ( orderData, setOrderFailedError, previousRequestError ) => {
	let response = {
		status: 'ok',
	};
	...
	try {
		const request = await fetch( `/api/update-order?orderID=${orderData.orderID}`, {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json',
			},
		...
		if ( result.error ) {
			response.error = result.error;
			setOrderFailedError( 'Something went wrong. Order updated failed. Please try again' );
		}
		response.orderId = result?.orderId ?? '';
		response.total = result.total ?? '';
		response.currency = result.currency ?? '';
